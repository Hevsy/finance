...
def index():
    ...
    with engine.begin() as db:
        assets = db.execute(text(
            "SELECT symbol, amount FROM assets WHERE user_id= :id"), {"id": session["user_id"]})
...
def buy():
    ...
    if request.method == "POST":
        ...
        with engine.begin() as db:
            ...
            a = db.execute(text(
                "SELECT amount FROM assets WHERE user_id = :id AND symbol=:sy"), {"id": session["user_id"], "sy": symbol})
            if not a:
...
def history():
    ...
    with engine.begin() as db:
        history = db.execute(text(
            "SELECT symbol, amount, ppu, datetime FROM transactions WHERE user_id = :id"), {"id": session["user_id"]})
        # print(history)
...
def sell():
    ...
    if request.method == "GET":
        with engine.begin() as db:
            assets = db.execute(text(
                "SELECT symbol FROM assets WHERE user_id=?"), {"id": session["user_id"]})
